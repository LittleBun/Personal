...
void print_mat(float *A, int size) {
   int i, j;
   for (i = 0; i < size; i++) {
      for (j = 0; j < size; j++) {
...
int main(int argc, char *argv[])
{
   ...
   if ((A = malloc(sizeof(float) * local_size*local_size)) == NULL ||
         (B = malloc(sizeof(float) * local_size*local_size)) == NULL ||
         (C = calloc(local_size*local_size, sizeof(float))) == NULL ||
         (bufferA = malloc(sizeof(float) * local_size*local_size)) == NULL ||
         (bufferB = malloc(sizeof(float) * local_size*local_size)) == NULL ) {
   ...
   */
   ...
   for (int k=0; k<sqrt_num_procs; k++) {
      ...
      // Broadcast
      if (k==coords[1]) {
         MPI_Bcast(A, local_size*local_size, MPI_FLOAT, k, row_comm);
      } else {
         MPI_Bcast(bufferA, local_size*local_size, MPI_FLOAT, k, row_comm);
      }
      if (k==coords[0]) {
         MPI_Bcast(B, local_size*local_size, MPI_FLOAT, k, col_comm);
      } else {
         MPI_Bcast(bufferB, local_size*local_size, MPI_FLOAT, k, col_comm);
      }
   ...
   // Test total sum
   float ideal = size*size*size*(size-1)*(size-1)*0.5;
   float total = 0.0;
   MPI_Reduce(&sum, &total, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
   if(rank == 0) {
      printf("The sum is %f\n", total);
      if (total-ideal > E || total-ideal < -E) {
         printf("Error!\n");
      }
   }
#ifdef DEBUG
